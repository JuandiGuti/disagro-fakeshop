...
async function exportOrderPDF(order) {
  ...
  const created = order.createdAt
    ...
    : "-";
  doc.text(`Pedido: ${String(order._id)}`, 14, 28);
  doc.text(`Fecha: ${created}`, 14, 34);
  doc.text(`Cupón: ${order.couponCode || "—"}`, 14, 40);
  ...
  doc.setFontSize(12);
  doc.text(`Subtotal: $${Number(order.subtotal ?? 0).toFixed(2)}`, 14, y);
  y += 6;
  doc.text(`Descuento: - $${Number(order.discount ?? 0).toFixed(2)}`, 14, y);
  y += 6;
  doc.setFontSize(14);
  doc.text(`Total: $${Number(order.total).toFixed(2)}`, 14, y);

  y += 12;
  doc.setFontSize(10);
  doc.text("Gracias por su compra.", 14, y);

  const filename = `pedido-${String(order._id)}.pdf`;
  doc.save(filename);
}

export default function OrdersClient({ initialOrders = [], initialError = "" }) {
  const card = {
              ...
              <tbody>
                {o.items.map((it, idx) => (
                  <tr key={idx} style={{ borderTop: "1px solid #e5e5e5" }}>
                    ...
                    <td style={{ textAlign: "center", padding: 10 }}>{it.qty}</td>
                    <td style={{ textAlign: "center", padding: 10 }}>
                      ${(Number(it.price) * Number(it.qty)).toFixed(2)}
                    </td>
                    <td style={{ textAlign: "center", padding: 10 }}>
